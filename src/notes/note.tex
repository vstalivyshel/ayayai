...
\begin{document}
    ...
    \begin{align}
        ...
        C'(w)
           ...
           &= \frac{1}{n}\sum_{i=1}^{n}\left((x_iw - y_i)^2\right)' =\\ % applying (f(g(x)))' = f'(g(x)) * g'(x) 'chain rule'
           &= \frac{1}{n}\sum_{i=1}^{n}((x_iw - y_i)^2)'(x_iw - y_i)' =\\ % applying (x^n)' = n*x^n-1 'power rule'
    ...
    \begin{align}
     ...
     \begin{align}
         ...
         \pd[w_1]C
            ...
            &= \avgsum[i,n]2(a_1 - z_i)\pd[w_1](a_1 - z_i) =\\ % z is a constant
	...
	\begin{align}
    	...
    	\pd[a_i^{(1)}]a_i^{(2)}
    		&= a_i^{(2)}(1 - a^{(2)})w^{(2)} \\
    	\pd[w^{(2)}] a_i^{(2)}
    		% &= \pd[w^{(2)}] (\sigma(a_i^{(1)}w^{(2)} + b^{(2)})) \\
    		% &= \pd[w^{(2)}]\sigma(a_i^{(1)}w^{(2)} + b^{(2)}) \pd[w^{(2)}](a_i^{(1)}w^{(2)} + b^{(2)}) \\
    		% &= \sigma(a_i^{(1)}w^{(2)} + b^{(2)})(1 - \sigma(a_i^{(1)}w^{(2)} + b^{(2)})) \pd[w^{(2)}](a_i^{(1)}w^{(2)} + b^{(2)}) \\
    		% &= \sigma(a_i^{(1)}w^{(2)} + b^{(2)})(1 - \sigma(a_i^{(1)}w^{(2)} + b^{(2)})) a_i^{(1)} \\
    		&= a_i^{(2)}(1 - a_i^{(2)}) a_i^{(1)} \\
    	\pd[w^{(2)}] a_i^{(2)}
    		&= a_i^{(2)}(1 - a_i^{(2)}) \\
    	\pd[w^{(2)}] C^{(2)}
    		% &= \avgsum[i, n] \pd[w^{(2)}] ((a_i^{(2)} - y_i)^2) =\\
    		% &= \avgsum[i, n] \pd[w^{(2)}] (a_i^{(2)} - y_i)^2 \pd[w^{(2)}] (a_i^{(2)} - y_i) =\\
		...
		e_i &= a_i^{(1)} - \pd[a_i^{(1)}] C^{(2)} \\ % expected output of the previous layer
